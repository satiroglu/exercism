...
CLASS zcl_itab_basics IMPLEMENTATION.
  METHOD fill_itab.
    "add solution here // placing 6 records into the internal table
  initial_data = VALUE #( 
  ...
  METHOD add_to_itab.
    ...
    "add solution here // adding a record to the end of the internal table
    append value #( group = 'A'   number = 19    description = 'Group A-4' ) to updated_data.
  ENDMETHOD.

  METHOD sort_itab.
    updated_data = initial_data.
    "add solution here // sorting the internal table by group and number
  sort updated_data 
      by 
        group ascending 
        number descending.
